...
- **Read aloud** — 🔊 toolbar button or Ctrl+R speaks the selected text (or
  the whole page) using the built-in Windows voice; click again to stop
...
└── ui/
    ...
    ├── pdf_viewer.py      # pypdfium2-based viewer with overlays
...
Dependencies: **PySide6** (UI, LGPL), **pypdfium2** (rendering & search, BSD/Apache), **pypdf** (PDF edits, BSD),
...
MIT License. All dependencies are permissively licensed (LGPL/BSD/Apache/MIT)
— see [THIRDPARTY.md](THIRDPARTY.md) for the full list and the LGPL
dynamic-linking note for PySide6. Earlier development versions used PyQt5
(GPL) and PyMuPDF (AGPL); both have been fully replaced.
